...
RUN \
    ...
    c2c-template --vars ${VARS_FILE} \
        --get-config tb_geoportal/config.yaml \
        ${CONFIG_VARS} && \
    pykwalify --data-file tb_geoportal/config.yaml \
        --schema-file CONST_config-schema.yaml && \
    rm CONST_* vars.yaml && \
    qgisserver-plugin-config tb_geoportal/config.yaml ../qgisserver/geomapfish.yaml.tmpl
...
RUN \
    ...
    mv /tmp/config/geoportal/tb_geoportal/ /etc/geomapfish/ && \
